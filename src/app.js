...
let inventarioSinTrampa =[];
const valorAEliminar = 'trampa';
inventario.forEach((objeto) =>{
  if (objeto !== valorAEliminar) 
    inventarioSinTrampa.push(objeto);
});

inventarioSinTrampa.push("super estrella");
let inventarioEnMayusculas = [];
for (let i = 0; i < inventarioSinTrampa.length; i++) {
  let mayusculas = inventarioSinTrampa[i].toUpperCase();
  inventarioEnMayusculas.push(mayusculas);
}
...
for (let obj = 0; obj < inventario.length; obj++) {
  if (inventario[obj] === "trampa") 
    console.log("¡Cuidado! Hay una trampa");
  else if (inventario[obj] === "estrella")
    console.log("¡Una estrella! Usar en caso de emergencia");
  else console.log(`Mario tiene un(a) ${inventario[obj]}.`);
}
